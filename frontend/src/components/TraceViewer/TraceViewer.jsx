...
export default function TraceViewer({ trace }) {
  ...
  if (trace.crashed) {
    return (
      <div className="trace-viewer">
        <p className="error">Trace could not complete (code crashed or timed out).</p>
        {trace.stderr && <pre className="error">{trace.stderr}</pre>}
      </div>
    );
  }
  if (steps.length === 0) {
    return <p className="muted">No trace steps captured (code may not have executed any lines).</p>;
  }
  ...
  return (
    <div className="trace-viewer">
      ...
      {trace.truncated && (
        <p className="warning">Trace was truncated (too many steps) -- showing a prefix only.</p>
      )}
