...
## Phase 1 pivot: 32 curated problems shipped, not ~90-95

The original `problem-roadmap.md` sketch (written pre-implementation) targeted ~90-95 problems. What actually shipped, fully built and verified, is **32 problems** -- a deliberate scope tradeoff made when the plan changed to "finish a usable app before the Claude subscription ends" (see the pivot instructions). Reasoning: every seeded problem here has a hand-written description, constraints, starter code, a verified reference solution, edge-case notes, 3 real hints, and multiple auto-verified test cases -- getting one problem to that bar properly is real work, and doing it ~90 times at the same rigor was not realistic alongside also shipping the trace visualizer, the specialized DSA visualizers, the learning-intelligence features, the dashboard UI, and mock-interview mode in the same remaining time. 32 problems, chosen to cover all 15 requested interview patterns (arrays/strings, hashing, two pointers, sliding window, prefix sums, sorting, binary search, recursion/backtracking, linked lists, stacks/queues, trees/BSTs, heaps, graphs, BFS/DFS, intro DP) with an Easy-before-Medium pair on the highest-frequency patterns, was judged the higher-value use of that time than a larger but thinner set. See `problem-roadmap.md`'s "Actual implementation" section for exactly which patterns got a full pair and which (heaps, graphs) are still single-problem and disclosed as a known gap, plus the reference frameworks used (NeetCode 150, Striver's sheet, LeetCode citations) and the in-place-mutation seeding bug this curation process caught and fixed.
